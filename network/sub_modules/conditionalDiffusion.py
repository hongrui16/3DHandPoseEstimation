...
class CustomUpsample(nn.Module):
    ...
    def forward(self, x):
        # print(f'CustomUpsample x.shape: {x.shape}')
        # Check if the input size is an odd number
        upsample_size = x.size(-1) * 2
        if x.size(-1) % 2 != 0:
            # If it is an odd number, adjust the upsampling size
            upsample_size += 1
...
class Unet1D(nn.Module):
    def __init__(
        ...
        if not condition_feat_dim is None:
            self.condition_feat_mlp = nn.Sequential(
                nn.Linear(condition_feat_dim, time_dim),
                nn.GELU(),
                nn.Linear(time_dim, time_dim)
            )
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                CustomUpsample(dim_out, dim_in) if not is_last else  nn.Conv1d(dim_out, dim_in, 3, padding = 1)                
            ]))
    ...
    def forward(self, x, time, x_self_cond = None, condition = None):
        ...
        for block1, block2, attn, downsample in self.downs:
            x = block1(x, t)
            # print(f'01 x.shape: {x.shape}')               
            h.append(x)

            x = block2(x, t)
            x = attn(x)
            # print(f'02 x.shape: {x.shape}')               
            h.append(x)
        ...
        for block1, block2, attn, upsample in self.ups:
            ...
            # print(f'0 h.pop().shape: {(h.pop()).shape}')
            ...
            h_pop = h.pop()
            # print(f'1 x.shape: {x.shape}')                        
            # print(f'1 h_pop.shape: {h_pop.shape}')
            x = torch.cat((x, h_pop), dim = 1)
            x = block1(x, t)

            h_pop = h.pop()
            # print(f'2 x.shape: {x.shape}')                        
            # print(f'2 h_pop.shape: {h_pop.shape}')
            x = torch.cat((x, h_pop), dim = 1)
            x = block2(x, t)
            x = attn(x)
...
class GaussianDiffusion1D(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise, condition = None):
        return (
            extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t -
            extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * noise
        )

    def predict_noise_from_start(self, x_t, t, x0, condition = None):
        return (
            (extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t - x0) / \
            extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape)
        )

    def predict_v(self, x_start, t, noise, condition = None):
        return (
            extract(self.sqrt_alphas_cumprod, t, x_start.shape) * noise -
            extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * x_start
        )

    def predict_start_from_v(self, x_t, t, v, condition = None):
        return (
            extract(self.sqrt_alphas_cumprod, t, x_t.shape) * x_t -
            extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * v
        )

    def q_posterior(self, x_start, x_t, t, condition = None):
        posterior_mean = (
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond = None, clip_denoised = True, condition = None):
    ...
    def p_losses(self, x_start, t, noise = None, condition = None):
        ...
        # print(f'x.shape: {x.shape}, t.shape: {t.shape}') # x.shape: torch.Size([4, 32, 128]), t.shape: torch.Size([4])
        # print(f'x_self_cond.shape: {x_self_cond.shape}') # NoneType
        model_out = self.model(x, t, x_self_cond, condition = condition)
        # print(f'model_out.shape: {model_out.shape}') # torch.Size([4, 32, 128])
