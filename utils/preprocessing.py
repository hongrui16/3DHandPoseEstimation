...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type):
    '''
    function:
        do augmentation for input image and 2d joint coordinates (uv), including rotation, scaling, translation, flipping, color scaling
        return the augmented image, joint coordinates (uvz), hand type, and the inverse transformation matrix.
    input:
        joint_coord: (42, 3), uvz
        joint_valid: (42,), 0 or 1
        hand_type: (2,), 0 or 1
        .....
    output:
        img: (256, 256, 3)
...
def transform_input_to_output_space(joint_coord, joint_valid, rel_root_depth, root_valid, root_joint_idx, joint_type):
    '''
    Transforms joint coordinates from input image space to output heatmap space. 
    Modify uvz, and allign the root joint depth to 32.
    
    This function adjusts the coordinates of joints (uvz) based on the configuration settings for input and output dimensions, 
    scales the depth values relative to the root joint, and updates the validity of joint and root positions based on their transformed coordinates.
    
    input:
    - joint_coord (np.ndarray): [42, 3], uvz coordinates of the joints.
    - joint_valid (np.ndarray): [42], A binary array indicating the validity of each joint.
    - rel_root_depth (float): [1], The relative depth of the root joint, always 0 for RHD
    - root_valid (np.ndarray): [1], a binary value indicating the validity of the root joint, always 0 for RHD
    - root_joint_idx (dict): A dictionary containing the indices of the root joints for the left and right hands.
    - joint_type (dict): A dictionary categorizing each joint as belonging to the left or right hand.
    
    Returns:
    - joint_coord (np.ndarray): [42, 3], uvz coordinates of the joints.
    - joint_valid (np.ndarray): [42], A binary array indicating the validity of each joint.
    - rel_root_depth (float): [1], The relative depth of the root joint, always 32 for RHD
    - root_valid (np.ndarray): [1], a binary value indicating the validity of the root joint, always 0 for RHD
